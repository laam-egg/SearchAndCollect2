...
struct _LkList {
    ...
    size_t size;
    size_t dataSize;
    LkElementFreeCallback freeElement;
};
...
LkList* lkInit(size_t dataSize, LkElementFreeCallback elementFreeCallback) {
    ...
    ll->freeElement = elementFreeCallback;
    return ll;
}
...
void lkRemove(LkList* ll, LkNode* which) {
    ...
    if (ll->freeElement != NULL) {
        ll->freeElement(lkGetNodeDataPtr(ll, which));
    }
    free((void*)which);
    --ll->size;
}
...
void lkClear(LkList* ll) {
    ...
    while (lkNext(&current)) {
        if (before != NULL) {
            if (ll->freeElement) {
                ll->freeElement(lkGetNodeDataPtr(ll, before));
            }
            free((void*)before);
        }
        before = current;
    ...
    if (current != NULL) {
        if (ll->freeElement) {
            ll->freeElement(lkGetNodeDataPtr(ll, current));
        }
        free((void*)current);
    }
